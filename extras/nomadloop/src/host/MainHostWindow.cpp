...
public:
			...
			controlSurface(new ControlSurfaceComponent())			
	{
		...
		playHeadLabel->setFont(Font(T("BoomBox 2"), 16, Font::plain));
	...
	void resized()
	{
		graphDocument->setSize(getWidth(), getHeight());
		controlSurface->setSize(getWidth(), getHeight());
	}

	void toggleFocusedComponent()
	{
		if (focusedComponent == graphDocument)
		{
			controlSurface->setVisible(true);
			graphDocument->setVisible(false);
			focusedComponent = controlSurface;
		}
		else
		{
			graphDocument->setVisible(true);
				  ...
				  DocumentWindow::allButtons)
{
	...
	ContentComp* contentComp = new ContentComp (&deviceManager);
...
{
    if (getGraphEditor() != 0
        && projectDocument->saveIfNeededAndUserAgrees() == FileBasedDocument::savedOk)
    {
...
{
    const wchar_t* const names[] = { T("File"), T("View"), T("Plugins"), T("Options"), 0 };
...
const PopupMenu MainHostWindow::getMenuForIndex (int topLevelMenuIndex, const String& /*menuName*/)
{
	...
	{
		menu.addCommandItem (commandManager, CommandIDs::toggleView);
	}
...
void MainHostWindow::menuItemSelected (int menuItemID, int /*topLevelMenuIndex*/)
{
    ...
    else if (menuItemID >= 100 && menuItemID < 200)
    {
        ...
        if (graphEditor != 0 && projectDocument->saveIfNeededAndUserAgrees() == FileBasedDocument::savedOk)
            projectDocument->loadFrom (recentFiles.getFile (menuItemID - 100), true);
    }
...
void MainHostWindow::getCommandInfo (const CommandID commandID, ApplicationCommandInfo& result)
{
	...
	case CommandIDs::toggleView:
		result.setInfo (T("Toggle view"),
						T("Toggles between patchbay and control surface view"),
						category, 0);
		// TODO: this is a bug, tab key don't work in ApplicationManager
		result.addDefaultKeypress (KeyPress::tabKey, 0);
		//result.addDefaultKeypress (T('l'), 0);
		break;
...
bool MainHostWindow::perform (const InvocationInfo& info)
{
    ...
    case CommandIDs::saveAs:
        if (graphEditor != 0)
			projectDocument->saveAs (File::nonexistent, true, true, true);
        break;

	case CommandIDs::toggleView:
		dynamic_cast<ContentComp*>(getContentComponent())->toggleFocusedComponent();
		break;
...
void MainHostWindow::showAudioSettings()
{
    ...
    audioSettingsComp.setSize (500, 540);
